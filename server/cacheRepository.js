...
export async function getReferencePlanningRows({
  startDate = '2025-07-01',
  endDate = '2025-12-31',
  referencias = []
} = {}) {
  ...
  const refs = referencias.map(ref => String(ref).trim()).filter(Boolean);
  ...
  const result = await query(`
    ...
    where v.cache_version = $1
      and v.start_date = $2::date
      and v.end_date = $3::date
      and p.referencia = any($4::text[])
    order by p.familia, p.referencia, p.cor, p.tamanho, v.empresa
  `, [CACHE_VERSION, startDate, endDate, refs]);

  return result.rows;
}

export async function getFamilyReferenceRows({
  ...
  const familiaList = familias.map(value => String(value).toUpperCase().trim()).filter(Boolean);
  const referenciaList = referencias.map(value => String(value).trim()).filter(Boolean);
  const colecaoList = colecoes.map(value => String(value).toUpperCase().trim()).filter(Boolean);
  const continuidadeList = continuidades.map(value => String(value).toUpperCase().trim()).filter(Boolean);

  if (familiaList.length === 0 && referenciaList.length === 0) return [];
  ...
  const result = await query(`
    ...
    where v.idempresa <> 1
      ...
      and ($3::text[] = '{}'::text[] or upper(trim(cl.familia)) = any($3::text[]))
      and ($4::text[] = '{}'::text[] or f_dic_prd_nivel(v.idproduto, 'CD'::bpchar) = any($4::text[]))
      and ($5::text[] = '{}'::text[] or upper(trim(cl.colecao)) = any($5::text[]))
      and ($6::text[] = '{}'::text[] or upper(trim(coalesce(cl.continuidade, ''))) = any($6::text[]))
    ...
    having sum(v.qt_liquida) <> 0
  `, [startDate, endDate, familiaList, referenciaList, colecaoList, continuidadeList]);
